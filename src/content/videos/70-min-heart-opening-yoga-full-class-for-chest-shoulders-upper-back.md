---
title: "70-min Heart-Opening Yoga - Full Class for Chest, Shoulders, Upper Back (Yin Yoga Challenge Class 3)"
display_title: "70-Minute Heart-Opening Yin"
subtitle: "A full class for the chest, shoulders and upper back"
slug: "70-min-heart-opening-yoga-full-class-for-chest-shoulders-upper-back"
youtube_id: "EFxCFzjorpg"
length_minutes: 70
display_length: "70 min"
watch_hours: 587
published: "2023-03-12"
enriched: true
level: "all-levels"
intent_tags: ["shoulders", "back", "energy"]
body_areas: ["chest", "shoulders", "spine", "neck"]
props: ["bolster", "block", "blanket"]
poses_featured: ["supported-fish", "thread-the-needle", "melting-heart", "broken-wing", "cow-face", "reclined-twist"]
chapters:
  - { time: "0:00",     seconds: 0,    title: "Welcome to heart-opening Yin" }
  - { time: "1:50",     seconds: 110,  title: "Rest · Corpse Pose",                            pose: "corpse" }
  - { time: "4:30",     seconds: 270,  title: "Chest · Supported Fish",                        pose: "supported-fish" }
  - { time: "15:00",    seconds: 900,  title: "Shoulders · Child's Pose & Thread-the-Needle", poses: ["childs-pose", "thread-the-needle"] }
  - { time: "26:00",    seconds: 1560, title: "Chest · Melting Heart",                         pose: "melting-heart" }
  - { time: "31:00",    seconds: 1860, title: "Shoulders · Broken Wing (each side)",           pose: "broken-wing" }
  - { time: "41:00",    seconds: 2460, title: "Shoulders · Cow-Face (each side)",              pose: "cow-face" }
  - { time: "50:24",    seconds: 3024, title: "Twist · Reclined Twist (each side)",            pose: "reclined-twist" }
  - { time: "57:12",    seconds: 3432, title: "Corpse Pose / Savasana",                        pose: "corpse" }
  - { time: "1:08:51",  seconds: 4131, title: "Goodbye · See you next week" }
seo_title: "70-Min Heart-Opening Yin Yoga — Chest, Shoulders, Back"
seo_description: "A 70-minute Yin Yoga class to open the chest, shoulders and upper back — gentle backbends and shoulder stretches held long and slow to release tension. All levels."
membership_cta: "Inside the membership, this class is part of a guided Full-Body program — so instead of choosing a video each day, you follow the plan and move through your week feeling looser and lighter."
summary: "A 70-minute Yin class to open the chest, shoulders and upper back — gentle backbends and shoulder stretches held long and slow to release tension. All levels."
faq:
  - q: "Do I need yoga blocks for this class?"
    a: "Blocks are handy for the supported fish, where you rest back over them to open the chest. If you don't have any, a few thick books work just as well — and a pillow or cushion and a blanket will cover the rest of the class."
  - q: "Is this heart-opening class suitable for beginners?"
    a: "Yes, it's all levels. The backbends and shoulder openers can feel intense, so find your edge at about 80% and take the gentler option in any pose. Read the pose notes first if you're new, and come out slowly."
  - q: "Why might a heart-opening practice feel emotional?"
    a: "Opening through the chest and front of the body can release stored tension, and that can feel surprisingly emotional. It's completely normal — give yourself plenty of space, keep breathing, and let whatever comes up simply pass."
---

## What this Yin class is

Seventy minutes of Yin to open through the front of the body. We gently backbend to release tension and blockages from the chest, then work slowly into the shoulders and upper back — supported fish, melting heart, broken wing and cow-face — with long, quiet holds in between. It's a lovely counter to hours spent hunched over a desk or a phone, and heart-opening can feel surprisingly emotional too, so give yourself plenty of space. This is class three of my nine-week Yin Yoga Challenge. Come as you are — every pose has a gentler option and a deeper one.

## Who this class is for

Anyone who sits or rounds forward a lot through the day and wants to open the chest, shoulders and upper back. It suits all levels — whether you're following the whole Yin Yoga Challenge week by week, or you just want to undo some tension and breathe more freely. New to Yin? Read the pose notes below first, then follow along at your own pace.

## What you need

Your mat, and a pillow or cushion and a blanket for support. A couple of yoga blocks are handy for the supported fish — a few thick books work just as well if you don't have blocks. A bolster is lovely if you have one. Set up somewhere warm and quiet, and press pause whenever you need to.

## How Yin Yoga works — three simple cues

- **Find your edge.** Come to about 80% of your capacity — a real stretch, but never forcing or straining. Backbends and shoulder openers can feel intense, so ease in gently.
- **Hold for time.** Stay a few minutes and let the shape do the work. Come out sooner if you need to, and come out slowly.
- **Be still.** Settle body and mind, and rest your attention on your breath — letting the chest soften and open with each exhale.
</content>
</invoke>
