---
title: "Total Body Deep Stretch | Full-Body Yoga Stretch (45-min yin yoga)"
display_title: "Total Body Deep Stretch"
subtitle: "A 45-minute full-body Yin stretch · long, deep holds"
slug: "total-body-deep-stretch-full-body-yoga-stretch-45-min-yin-yoga"
youtube_id: "P98tKAQR0DY"
length_minutes: 45
display_length: "45 min"
watch_hours: 319
published: "2023-01-18"
enriched: true
level: "all-levels"
intent_tags: ["full-body", "flexibility", "hips"]
body_areas: ["spine", "hips", "shoulders", "hamstrings"]
props: ["strap", "cushion or blocks"]
poses_featured: ["childs-pose", "melting-heart", "swan", "shoelace", "cow-face", "reclined-twist"]
chapters:
  - { time: "0:00",   seconds: 0,    title: "Welcome to total body deep stretch" }
  - { time: "1:20",   seconds: 80,   title: "Back · Child's Pose",                   pose: "childs-pose" }
  - { time: "8:00",   seconds: 480,  title: "Shoulders · Melting Heart",             pose: "melting-heart" }
  - { time: "12:00",  seconds: 720,  title: "Hips · Swan (side 1)",                  pose: "swan" }
  - { time: "15:40",  seconds: 940,  title: "Hips · Shoelace",                       pose: "shoelace" }
  - { time: "20:00",  seconds: 1200, title: "Shoulders · Cow Face arms",             pose: "cow-face" }
  - { time: "24:50",  seconds: 1490, title: "Hips · Swan & Cow Face arms (side 2)", poses: ["swan", "cow-face"] }
  - { time: "34:30",  seconds: 2070, title: "Twist · Reclined Twist (each side)",    pose: "reclined-twist" }
  - { time: "40:00",  seconds: 2400, title: "Corpse Pose / Savasana", poses: ["corpse"] }
pin_angles:
  - audience: "for tight hips"
    headline: "Forty-five minutes, everything"
    proof: "Follow along on YouTube. No props needed."
seo_title: "45-Minute Yin Yoga — Total Body Deep Stretch"
seo_description: "A 45-minute Yin Yoga class for a full-body deep stretch — long, slow holds through the spine, hips and shoulders to open and release. All levels."
membership_cta: "Love a deep, full-body stretch like this? Inside the membership it lives as part of a guided Full-Body program — so instead of choosing a class each day, you press play and follow the plan. Become someone who moves through the week feeling looser and lighter."
summary: "A 45-minute Yin class for a full-body deep stretch — long, slow holds through the spine, hips and shoulders to open and release. All levels."
faq:
  - q: "How long do we hold each pose?"
    a: "Around two to four minutes each. The long holds are what let the deeper connective tissue release, so let the shape do the work and stay as still as you comfortably can."
  - q: "Do I need any props for this class?"
    a: "A strap or belt is handy for the Cow Face arms and a cushion or a couple of blocks are lovely for sitting, but none of it is essential. If you don't have props, you can still do the whole class — every pose has a gentler option."
  - q: "Is this class suitable for beginners?"
    a: "Yes. It's an all-levels practice. Come to about 60–80% of your capacity rather than your deepest stretch, ease into each shape slowly, and come out of anything that feels sharp."
---

## What this Yin class is

A 45-minute full-body deep stretch in the style of Yin — where we hold each pose for around two to four minutes and let the long holds do the work. Yin works on the deeper connective tissue around your joints — the fascia, ligaments and tendons — rather than the muscles, so over time you build more range of motion and more stability. We move slowly through the spine, hips and shoulders, opening and releasing as we go. Come as you are — every pose has a gentler option and a deeper one.

## Who this class is for

Anyone craving a proper, unhurried stretch through the whole body — after a long day, on a stiff morning, or any time you want to feel loose and settled again. It suits every level, so whether you're brand new to Yin or well practiced, you can follow along and find your own depth. New to Yin? Read the pose notes below first, then move at your own pace.

## What you need

Just your mat, and a warm, quiet space where you can be still for a while. A strap or belt is handy for the Cow Face arms, and a cushion or a couple of blocks are lovely to sit on — but if you don't have them, you'll still be able to do the whole class. Press pause whenever you need to.

## How Yin Yoga works — three simple cues

- **Find your edge.** Come to about 60–80% of your capacity — a real stretch, but never forcing or straining. Ease in slowly, especially if you're new.
- **Hold for time.** Stay a few minutes and let the shape do the work; the length is where the change happens. Come out sooner if you need to.
- **Be still.** Settle body and mind, and rest your attention on the sensations and your breath.
