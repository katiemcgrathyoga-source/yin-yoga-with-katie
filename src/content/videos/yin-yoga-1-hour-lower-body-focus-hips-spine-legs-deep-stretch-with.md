---
title: "Yin Yoga 1 Hour - Lower Body Focus: Hips, Spine, Legs Deep Stretch with Long Holds."
display_title: "1-Hour Lower Body Yin"
subtitle: "Deep hip, spine & leg stretch · long holds"
slug: "yin-yoga-1-hour-lower-body-focus-hips-spine-legs-deep-stretch-with"
youtube_id: "Xi-NEkLr_0U"
length_minutes: 60
display_length: "60 min"
watch_hours: 317
published: "2024-05-11"
enriched: true
level: "all-levels"
intent_tags: ["hips", "back", "flexibility"]
body_areas: ["hips", "legs", "spine", "lower back"]
props: ["block"]
poses_featured: ["childs-pose", "dragon", "sleeping-swan", "shoelace", "melting-heart", "supported-fish", "reclined-twist"]
chapters:
  - { time: "0:00",   seconds: 0,    title: "Welcome to lower body Yin" }
  - { time: "1:00",   seconds: 60,   title: "Back · Extended Child's Pose",                pose: "childs-pose" }
  - { time: "6:00",   seconds: 360,  title: "Hips · Dragon (side 1)",                      pose: "dragon" }
  - { time: "11:00",  seconds: 660,  title: "Hips · Sleeping Swan (side 1)",               pose: "sleeping-swan" }
  - { time: "15:30",  seconds: 930,  title: "Hips · Shoelace, fold & twist (side 1)", poses: ["shoelace", "reclined-twist"] }
  - { time: "24:00",  seconds: 1440, title: "Hips · Dragon, Swan & Shoelace (side 2)", poses: ["dragon", "swan", "shoelace"] }
  - { time: "39:00",  seconds: 2340, title: "Spine · Melting Heart",                       pose: "melting-heart" }
  - { time: "44:20",  seconds: 2660, title: "Spine · Supported Fish",                      pose: "supported-fish" }
  - { time: "50:00",  seconds: 3000, title: "Twist · Reclined Twist (each side)",          pose: "reclined-twist" }
  - { time: "57:48",  seconds: 3468, title: "Corpse Pose / Savasana", poses: ["corpse"] }
pin_angles:
  - audience: "for a stiff back"
    headline: "An hour below the waist"
    proof: "Hips, legs and the low back only."
seo_title: "1-Hour Yin Yoga for Hips, Spine & Legs — Deep Stretch"
seo_description: "A 60-minute Yin Yoga class for the lower body — deep hip, spine and leg stretches held long and slow with long holds to release tension. All levels."
membership_cta: "Love a long, grounding stretch like this? Inside the membership it lives inside the Hips & Lower Back program — so instead of choosing a class each day, you press play and follow the plan. Become someone who moves through the day loose and easy."
summary: "A 60-minute Yin class for the lower body — deep hip, spine and leg stretches held long and slow with long holds, to release tension and unwind. All levels."
faq:
  - q: "What does this 1-hour Yin class focus on?"
    a: "The lower body — mainly the hips, legs and spine. The poses release tension through those areas and, over time, build a little more flexibility, while the long holds also help you unwind and relax."
  - q: "Do I need any props?"
    a: "Just a yoga block or two is handy — a block under the hips in Child's Pose or under the back in Supported Fish makes the shapes kinder. A cushion works just as well, and you can practice without props if you prefer."
  - q: "How long is each pose held?"
    a: "Yin poses here are held for at least a couple of minutes each, with the hip poses repeated on both sides. The hold times are only suggestions — listen to your body and come out of any pose sooner if you need to."
---

## What this Yin class is

A full hour of Yin for the lower body — a deep, grounding stretch through the hips, legs and spine. We move slowly through Child's Pose, Dragon, Sleeping Swan and a Shoelace fold and twist on each side, then open the spine with Melting Heart, Supported Fish and a Reclined Twist before resting. It's a lovely way to release tension from tight hips and a stiff back, gain a little flexibility, and simply unwind. Come as you are — every pose has a gentler option and a deeper one.

## Who this class is for

Anyone carrying tension in the hips, legs or lower back — after long days at a desk, on your feet, or when everything below the waist feels tight. It's also a calm, meditative hour if you just want to slow down and relax. New to Yin? Read the pose notes below first, then follow the shapes at your own pace.

## What you need

Just your mat, and a warm, quiet space where you can be still for a while. A yoga block or two is handy — under the hips in Child's Pose, or under the back in Supported Fish — though a cushion works just as well. Press pause whenever you need to.

## How Yin Yoga works — three simple cues

- **Find your edge.** Come to about 60–80% of your capacity — a real stretch, but never forcing or straining.
- **Hold for time.** Yin poses are held at least a couple of minutes; the length is where the change happens. Come out sooner if you need to.
- **Be still.** Settle body and mind, and rest your attention on the sensations and your breath.
