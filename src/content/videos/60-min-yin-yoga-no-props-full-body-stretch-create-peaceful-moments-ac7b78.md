---
title: "60-Min Yin Yoga | No Props Full-Body Stretch | Create Peaceful Moments Yoga Challenge Class 6"
display_title: "60-Minute No-Props Yin"
subtitle: "A full-body stretch with breathwork · no props"
slug: "60-min-yin-yoga-no-props-full-body-stretch-create-peaceful-moments-ac7b78"
youtube_id: "uYEZfzimhbc"
length_minutes: 60
display_length: "60 min"
watch_hours: 496
published: "2024-10-04"
enriched: true
level: "all-levels"
intent_tags: ["full-body", "flexibility", "nervous-system"]
body_areas: ["spine", "shoulders", "hips", "hamstrings"]
props: []
poses_featured: ["caterpillar", "melting-heart", "bow-tie", "childs-pose", "deer", "reclined-twist"]
chapters:
  - { time: "0:00",   seconds: 0,    title: "Welcome to the practice" }
  - { time: "1:20",   seconds: 80,   title: "Breath · 4:8 breathing" }
  - { time: "6:40",   seconds: 400,  title: "Spine · Caterpillar & Cat-Cow",        pose: "caterpillar" }
  - { time: "14:20",  seconds: 860,  title: "Shoulders · Melting Heart",            pose: "melting-heart" }
  - { time: "20:00",  seconds: 1200, title: "Shoulders · Bow-Tie",                  pose: "bow-tie" }
  - { time: "28:30",  seconds: 1710, title: "Back · Child's Pose",                  pose: "childs-pose" }
  - { time: "35:30",  seconds: 2130, title: "Hips · Deer & Swan (each side)", poses: ["deer", "swan"] }
  - { time: "45:00",  seconds: 2700, title: "Twist · Reclined Twist (each side)",   pose: "reclined-twist" }
  - { time: "54:40",  seconds: 3280, title: "Corpse Pose / Savasana", poses: ["corpse"] }
seo_title: "60-Minute Yin Yoga: No Props Full-Body Stretch & Breath"
seo_description: "A calming 60-minute Yin Yoga class — a no-props full-body stretch with breathwork and long, slow holds to create peaceful moments. All levels, nothing needed but your mat."
membership_cta: "Love a full hour of quiet like this? Inside the membership it lives as part of a guided Full-Body program — so instead of choosing a class each day, you press play and follow the plan. Become someone who moves through the week feeling looser and lighter."
summary: "A 60-minute Yin class — a no-props full-body stretch with breathwork and long, slow holds to create peaceful moments. All levels."
faq:
  - q: "Do I need any props for this class?"
    a: "No — this is a no-props class. All you need is your mat and a warm, quiet space. It's always nice to have a couple of blocks or a cushion on standby in case you'd like a little support under your head or knees, but nothing is required. Come as you are."
  - q: "Is this suitable for beginners?"
    a: "Yes. It's an all-levels class, and I name each pose and how long we'll stay before we settle in. Every pose has a gentler option and a deeper one, so find your edge at about 80% and let the shape do the work."
  - q: "How long do we hold each pose?"
    a: "Most poses are held for two to five minutes — the length is where the change happens. But the times are only a guide. Listen to your body, be still where you can, and come out slowly whenever you need to."
---

## What this Yin class is

A full hour of Yin to create some peaceful moments. We open with a gentle 4:8 breathing exercise to settle the nervous system, then move slowly through a full-body sequence — folding forward, opening the shoulders and upper back, softening into the hips, and unwinding through long reclined twists. I name each pose and how long we'll stay, then let you sink into the hold. This is class six of the Create Peaceful Moments series, but it stands completely on its own. Come as you are — every pose has a gentler option and a deeper one.

## Who this class is for

Anyone who wants to give themselves a proper hour on the mat — to stretch the whole body, quieten the mind, and drop out of a busy week. It's lovely after a long day, at the weekend, or any time you're craving a slower, more meditative practice. New to Yin? Read the pose notes below first, then follow along at your own pace.

## What you need

Just your mat, and a warm, quiet space where you can be still for a while. This is a no-props class, so nothing is required — though it's always nice to have a couple of blocks or a cushion on standby, in case you'd like extra padding under your knees or something to rest your head on. Press pause whenever you need to.

## How Yin Yoga works — three simple cues

- **Find your edge.** Come to about 80% of your capacity — a real stretch, but never forcing or straining.
- **Hold for time.** Yin poses are held a while; the length is where the change happens. Come out sooner if you need to.
- **Be still.** Settle body and mind, and rest your attention on the sensations and your breath.
