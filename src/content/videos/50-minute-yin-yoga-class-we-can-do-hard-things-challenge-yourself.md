---
title: "50-Minute Yin Yoga Class \"We Can Do Hard Things\" | Challenge Yourself & Find Inner Strength with Yin"
display_title: "We Can Do Hard Things Yin"
subtitle: "A deeper 50-minute full-body practice · challenge yourself"
slug: "50-minute-yin-yoga-class-we-can-do-hard-things-challenge-yourself"
youtube_id: "s58fQpubsC0"
length_minutes: 52
display_length: "50 min"
watch_hours: 249
published: "2024-11-15"
enriched: true
level: "intermediate"
intent_tags: ["full-body", "hips", "shoulders"]
body_areas: ["hips", "shoulders", "hamstrings", "lower back"]
props: ["yoga blocks", "strap"]
poses_featured: ["dragon", "cow-face", "shoelace", "squat", "dangling"]
chapters:
  - { time: "0:00",   seconds: 0,    title: "Welcome · Intro to We Can Do Hard Things" }
  - { time: "2:00",   seconds: 120,  title: "Hips · Dragon (each side)",           pose: "dragon" }
  - { time: "15:50",  seconds: 950,  title: "Shoulders · Cow-Face Arms & Shoelace", poses: ["cow-face", "shoelace"] }
  - { time: "30:50",  seconds: 1850, title: "Hips · Squat",                        pose: "squat" }
  - { time: "36:34",  seconds: 2194, title: "Legs · Dangling",                     pose: "dangling" }
  - { time: "43:40",  seconds: 2620, title: "Corpse Pose / Savasana", poses: ["corpse"] }
pin_angles:
  - audience: "for stiff shoulders"
    headline: "Fifty minutes, and it is not easy"
    proof: "Longer holds than usual. Stay anyway."
seo_title: "50-Min \"We Can Do Hard Things\" Yin Yoga | Full-Body"
seo_description: "A deeper 50-minute full-body Yin Yoga class to challenge yourself and find inner strength — longer, tougher holds in Dragon, Squat and Dangling. Intermediate."
membership_cta: "Loved being challenged like this? Inside the membership this class is part of a guided Full-Body program — so instead of choosing a video each day, you press play and follow the plan. Become someone who moves through the week feeling stronger, looser and lighter."
summary: "A deeper 50-minute full-body Yin class built to challenge you — longer, tougher holds in Dragon, Cow-Face arms, Squat and Dangling that teach you, on the mat, that you can do hard things. Intermediate."
faq:
  - q: "Is this class suitable for beginners?"
    a: "It's a deeper, intermediate practice. The poses are chosen from the more challenging end of Yin, with longer holds that ask for a little endurance. If you're brand new, that's okay — read the pose notes first, keep your blocks and strap handy for support, and come out of any shape whenever your body asks you to."
  - q: "What props do I need?"
    a: "Have a couple of yoga blocks and a strap nearby. Blocks support your hands in Dragon and give you a seat in Squat, and the strap helps with the Cow-Face arm bind. None of it is essential — every pose has a gentler option — but it makes the deeper holds far kinder."
  - q: "Where does \"We can do hard things\" come from?"
    a: "It's a phrase from the book Untamed by Glennon Doyle. On the mat we practice ignoring the voice that wants to give up, and that quiet strength tends to travel with you off the mat and into everyday life."
---

## What this Yin class is

Fifty minutes of Yin chosen to challenge you — and to remind you that you can do hard things. Because Yin holds poses for a long time, it can get uncomfortable, and a voice in your head starts saying *I can't do this, let's give up.* On the mat we practice ignoring that voice. I've picked poses from the deeper end of Yin — Dragon, Cow-Face arms, Squat and Dangling — shapes that build a little endurance and stamina, held long enough to test your resolve. The strength you find here has a way of following you off the mat. Come as you are — every pose has a gentler option and a deeper one.

## Who this class is for

Anyone ready to be challenged: you have a steady Yin practice and want longer, tougher holds, or you simply love the idea of proving to yourself that you can stay when things get hard. It's an intermediate class, but not a competition — you set the depth. New to Yin? Read the pose notes below first, keep your props close, and go at your own pace.

## What you need

Your mat, a couple of yoga blocks, and a strap. The blocks support your hands in Dragon and become a seat in Squat; the strap helps with the Cow-Face arm bind. A warm, quiet space where you can be still helps too. Press pause whenever you need to, and if your body clearly signals you to stop a pose, please do — that's always okay.

## How Yin Yoga works — three simple cues

- **Find your edge.** Come to about 60–80% of your capacity — a real stretch, but never forcing or straining.
- **Hold for time.** Yin poses are held a while, and the length is where the change happens. Come out sooner if you need to — the hold times are only suggestions.
- **Be still.** Settle body and mind, let the shape do the work, and rest your attention on the sensations and your breath. Come out slowly when it's time.
