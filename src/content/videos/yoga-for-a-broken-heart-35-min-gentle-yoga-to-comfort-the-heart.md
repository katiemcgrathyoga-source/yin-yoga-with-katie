---
title: "Yoga for a Broken Heart || 35-min Gentle Yoga to Comfort the Heart"
display_title: "Yoga for a Broken Heart"
subtitle: "35 minutes of gentle, heart-opening Yin to soften the chest and settle the mind"
slug: "yoga-for-a-broken-heart-35-min-gentle-yoga-to-comfort-the-heart"
youtube_id: "PEfNQTXZzHw"
length_minutes: 35
display_length: "35 min"
watch_hours: 110
published: "2023-07-20"
enriched: true
level: "all-levels"
intent_tags: ["nervous-system", "stress", "shoulders"]
body_areas: ["chest", "shoulders", "spine", "hips"]
props: ["cushion", "blocks"]
poses_featured: ["butterfly", "childs-pose", "melting-heart"]
chapters:
  - { time: "0:00",   seconds: 0,    title: "Welcome · Yoga for a broken heart" }
  - { time: "1:30",   seconds: 90,   title: "Breath · Opening meditation" }
  - { time: "6:30",   seconds: 390,  title: "Rest · Lie down and settle" }
  - { time: "10:20",  seconds: 620,  title: "Hips & chest · Reclined Butterfly", poses: ["butterfly"] }
  - { time: "15:30",  seconds: 930,  title: "Shoulders · Seated chest & shoulder opener" }
  - { time: "16:30",  seconds: 990,  title: "Spine · Child's Pose", poses: ["childs-pose"] }
  - { time: "20:00",  seconds: 1200, title: "Seated pause" }
  - { time: "21:00",  seconds: 1260, title: "Heart & upper back · Melting Heart", poses: ["melting-heart"] }
  - { time: "25:00",  seconds: 1500, title: "Neck · Neck stretches & warming hands" }
  - { time: "28:30",  seconds: 1710, title: "Cat-Cow to Downward Dog" }
  - { time: "32:00",  seconds: 1920, title: "Closing meditation" }
  - { time: "33:00",  seconds: 1980, title: "Like & subscribe" }
seo_title: "Yoga for a Broken Heart — 35 min Gentle Heart-Opening Yin"
seo_description: "A gentle 35-minute Yin Yoga class to comfort the heart — soft breathing, reclined butterfly, child's pose and melting heart to open the chest and settle the mind."
membership_cta: "If a slow, tender practice like this is what you reach for, the membership gathers classes like it into a guided program — so you can press play and follow the plan through the week instead of choosing a video each day."
summary: "A gentle 35-minute Yin class to comfort the heart — soft breathing, reclined butterfly, child's pose and melting heart to open the chest and quiet the mind."
faq:
  - q: "Do I need any props for this class?"
    a: "Nothing essential — just your mat. A pillow or cushion is lovely to have nearby, though: there are moments to hold it close or rest your head or forehead on it, and a couple of blocks or cushions are nice to sit up on if that's more comfortable for you. Come as you are."
  - q: "Is this class suitable if I'm not very flexible?"
    a: "Yes. This is a soft, gentle practice, and Yin isn't about how far you go. Find your edge at about 60–80%, let each shape do the work, and take the gentler option whenever you'd like. Everything here can be softened with a cushion or a bend in the elbows."
  - q: "I'm feeling emotional — is that okay in this practice?"
    a: "Completely. This class was made as a quiet, comforting place to land when the heart feels heavy. Move at your own pace, rest whenever you need to, and let the breath and the stillness hold you. There's nothing you need to do except be here."
---

## What this Yin class is

A gentle 35 minutes made for the days when the heart feels heavy. We begin seated with some soft, calming breathing, then settle onto our backs and open slowly through the chest and hips in a reclined butterfly. From there we fold into child's pose, melt the upper back open in melting heart, ease out the neck, and close with a little stillness. It's slow, cosy and undemanding from start to finish — a quiet place to land and breathe.

## Who this class is for

Anyone who wants a soft, comforting practice to move through when they're feeling tender, low or simply in need of some gentleness. It suits all levels, whether you're brand new to Yin or well practised — nothing here is strong or strenuous. If you'd like company and a calm voice to guide you, this is for you.

## What you need

Just your mat and a warm, quiet space where you can be still. A pillow or cushion nearby is lovely — to hold close, to rest your forehead on, or to sit up on — and a couple of blocks work well too if you have them. Every pose has a gentler option and a deeper one, so take whichever your body wants today and pause whenever you need to.

## How Yin Yoga works — three simple cues

- **Find your edge.** Come to about 60–80% of your capacity — a real stretch, but never forcing or straining.
- **Hold for time.** Yin poses are held a couple of minutes or more; the length is where the change happens. Come out sooner if you need to.
- **Be still.** Settle body and mind, and rest your attention on your breath — then come out slowly when it's time.
