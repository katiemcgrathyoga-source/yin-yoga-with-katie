---
title: "Yin Yoga For Shoulders and Upper Back | 5 Poses for Flexible Shoulders and Flexible Upper Back"
display_title: "Yin for Shoulders & Upper Back"
subtitle: "A 36 min shoulder and upper-back stretch · 5 poses, strap optional"
slug: "yin-yoga-for-shoulders-and-upper-back-5-poses-for-flexible-shoulders"
youtube_id: "M0Iz4VRaT_Q"
length_minutes: 36
display_length: "36 min"
watch_hours: 92
published: "2024-03-22"
enriched: true
level: "all-levels"
intent_tags: ["flexibility", "shoulders"]
body_areas: ["shoulders", "upper-back", "chest"]
props: ["strap", "block"]
poses_featured: ["melting-heart", "childs-pose", "bow-tie", "broken-wing", "crocodile", "eagle", "cow-face", "corpse"]
chapters:
  - { time: "0:00",  seconds: 0,    title: "Welcome · settle in for softer shoulders" }
  - { time: "0:40",  seconds: 40,   title: "Front of shoulders · Melting Heart to Child's Pose", poses: ["melting-heart", "childs-pose"] }
  - { time: "6:00",  seconds: 360,  title: "Sides of shoulders · Bow Tie to Broken Wing (each side)", poses: ["bow-tie", "broken-wing", "crocodile"] }
  - { time: "22:30", seconds: 1350, title: "Whole shoulder · Eagle Arms to Cow-Face Arms (each side)", poses: ["eagle", "cow-face"] }
  - { time: "34:00", seconds: 2040, title: "Rest · Corpse Pose when you're ready", poses: ["corpse"] }
pin_angles:
  - audience: "for stiff shoulders"
    headline: "Five poses for stuck shoulders"
    proof: "Thirty-six minutes, upper back and chest."
seo_title: "Yin Yoga For Shoulders & Upper Back — 36 min, All Levels"
seo_description: "A 36 min Yin Yoga class for flexible shoulders and a supple upper back — five long-held poses that move the shoulder joint in every direction. Strap optional, all levels."
membership_cta: "Love a gentle practice like this? Inside the membership it's part of a guided Shoulders program — press play and follow the plan instead of choosing a class each day, and let the week leave you feeling looser and lighter."
summary: "A 36 min Yin Yoga class for flexible shoulders and a supple upper back. Five poses move the shoulder joint in every direction, held long and slow to help release tight, stiff shoulders. A strap is handy but optional — suitable for all levels."
faq:
  - q: "Do I need a strap for this shoulder class?"
    a: "It's handy but not essential. We use a strap for the cow-face arms near the end, so your hands have something to hold if they don't quite meet behind your back. If you don't have one, don't worry — you can simply reach your hands toward each other instead. A yoga block or cushion is also nice to have for resting your forehead early on."
  - q: "Is this class okay if my shoulders feel really tight?"
    a: "Yes — that's exactly who it's for. Every pose has a lighter option, and I'll remind you to come to about 60 to 80 percent rather than forcing. If your hands or arms start to tingle in the first pose, simply bend your elbows to ease off. You choose how deep to go, and tight shoulders are welcome just as they are."
  - q: "How long are the poses held?"
    a: "A while — this is Yin, so we settle in and let gravity do the work. The opening poses are held around three minutes each, and the arm poses around two minutes per side. If a shape becomes too intense, come out slowly and rest whenever you need to. The length is where the softening happens."
---

## What this Yin class is

A 36-minute Yin practice to help open tight, stiff shoulders and free up your upper back. We move the shoulder joint in every direction with five simple poses — starting with Melting Heart to open the front of the shoulders and chest, rolling through Bow Tie and Broken Wing for the sides, and finishing with Eagle and Cow-Face arms to reach all the way around. Each pose is held long and slow, so the shape can quietly do its work. Come as you are — every pose has a gentler option and a deeper one, and you decide which feels right today.

## Who this class is for

Anyone carrying tension across the shoulders and upper back — after long days at a desk, hunched over a phone, or any time the area feels tight and stiff. It's lovely if you want to build a little more flexibility and ease through the shoulders without rushing. New to Yin? Read the pose notes below first, then follow the shapes at your own pace and rest whenever you need to.

## What you need

Your mat, and a strap if you have one — we use it for the cow-face arms near the end, though you can reach your hands toward each other instead. A yoga block or cushion is nice for resting your forehead in the opening poses, and folding the front of your mat gives the knees a little extra padding. None of it is essential, so work with whatever you have nearby.

## How Yin Yoga works — three simple cues

- **Find your edge.** Come to about 60–80% of your capacity — a real stretch through the shoulders, but never forcing or straining.
- **Hold for time.** Yin poses are held a while; let gravity do the work. Come out slowly, and sooner if you feel any tingling or it becomes too intense.
- **Be still.** Settle body and mind, rest your attention on your breath, and let each shape gently soften the area over time.
