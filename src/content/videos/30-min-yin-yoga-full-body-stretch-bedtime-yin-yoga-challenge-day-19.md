---
title: "30-Min Yin Yoga Full-Body Stretch | Bedtime Yin Yoga Challenge Day 19"
slug: "30-min-yin-yoga-full-body-stretch-bedtime-yin-yoga-challenge-day-19"
youtube_id: "vjaBOsAOOcg"
length_minutes: 34
display_length: "30 min"
watch_hours: 189
published: "2024-04-19"
enriched: true
level: "all-levels"
intent_tags: ["sleep", "full-body", "nervous-system"]
body_areas: ["spine", "shoulders", "hips", "hamstrings"]
props: ["none"]
poses_featured: ["half-butterfly", "thread-the-needle", "childs-pose", "reclined-twist"]
chapters:
  - { time: "0:00",   seconds: 0,    title: "Intro to Bedtime Yin" }
  - { time: "0:50",   seconds: 50,   title: "Hamstrings · Half-Butterfly",            poses: ["half-butterfly"] }
  - { time: "12:00",  seconds: 720,  title: "Shoulders · Thread-the-Needle & Child's Pose", poses: ["thread-the-needle", "childs-pose"] }
  - { time: "24:00",  seconds: 1440, title: "Spine · Reclined Twist",                 poses: ["reclined-twist"] }
  - { time: "32:40",  seconds: 1960, title: "Like & Subscribe!" }
seo_title: "30-Min Bedtime Yin Yoga — Full-Body Stretch for Sleep"
seo_description: "A calming 30-minute full-body Yin Yoga class to unwind before bed — half-butterfly, thread-the-needle, child's pose and a reclined twist. All levels, no props."
membership_cta: "This class lives inside the membership's Sleep program — a night-by-night path that helps you fall asleep faster and sleep through. Press play and let it carry you."
summary: "A gentle 30-minute bedtime Yin class — a full-body stretch to help you unwind, relax and settle before sleep."
faq:
  - q: "Do I need any props for this bedtime class?"
    a: "Not at all — this one is designed to be done with nothing but your mat, so you can roll it out beside your bed and press play. If you'd like a little extra comfort, a cushion or folded blanket to sit on or rest under is lovely, but it's entirely optional."
  - q: "Is this suitable if I'm not very flexible?"
    a: "Yes — Yin isn't about how far you go. Find your edge at about 60–80%, let the shape do the work, and take the gentler option in any pose whenever you need it. Over the nights of the challenge you'll notice things easing in their own time."
  - q: "What does Day 19 actually cover?"
    a: "We settle in with some calming breath, then hold a long half-butterfly for the backs of the legs, ease out the upper back and shoulders with thread-the-needle and child's pose, and finish in a slow reclined twist to unwind the spine before bed."
---

## What this Yin class is

Day 19 of the Bedtime Yin Challenge — a gentle 30-minute full-body stretch to help you unwind and let go of the day before sleep. We settle in with the breath, hold a long half-butterfly for the hamstrings and lower back, then ease the upper back and shoulders with thread-the-needle and a restful child's pose, before finishing in a slow reclined twist to release the spine. Slow, still and calming — the perfect way to end your day.

## Who this class is for

Anyone who'd like to relax, stretch and quiet the mind before bed. It's a lovely evening wind-down whether you're brand new to Yin or well practiced, and it fits beautifully into the 30-day bedtime challenge. New to this? Simply follow the shapes at your own pace and take the gentler option whenever you need it.

## What you need

Just your mat and a warm, quiet space where you can be still — ideally somewhere you can drift off afterwards. This is a no-props class, though a cushion or folded blanket nearby is nice to sit on or rest under if you'd like a little more support. Every pose has a gentler option and a deeper one, so make it yours.

## How Yin Yoga works — three simple cues

- **Find your edge.** Come to about 60–80% of your capacity — a real stretch, but never forcing or straining.
- **Hold for time.** Yin poses are held a couple of minutes or more; the length is where the change happens. Come out sooner if you need to.
- **Be still.** Settle body and mind, and rest your attention on your breath — then come out slowly when it's time.
