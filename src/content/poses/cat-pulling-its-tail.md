---
name_en: "Cat Pulling Its Tail"
slug: "cat-pulling-its-tail"
also_known_as: []
target_areas: ["spine", "lower back", "hips", "quadriceps", "hip flexors"]
meridians: ["gall bladder", "urinary bladder", "kidney", "stomach", "spleen"]
accessibility: "intermediate"
hold_time: "2–5 minutes per side"
hold_seconds: 120
props: ["strap", "cushion"]
benefits:
  - "Takes the wind-down of a reclined twist and adds a lovely stretch through the front of the thigh and hip flexor."
  - "Releases the lower back and outer hip while gently opening the quad of the extended leg."
  - "Wrings the spine and massages the belly, which can ease digestion."
  - "Opens the chest and shoulder as the top arm reaches back to catch the foot."
  - "A satisfying two-in-one shape — twist and front-line opener in a single pose."
cues:
  - "Begin lying on your back, then let both knees drop over to your left."
  - "Slide the bottom (left) leg long down the mat, keeping the top (right) knee bent and resting toward the floor."
  - "Reach back with your right hand and catch the left foot or ankle — loop a strap around it if it's out of reach."
  - "Draw the foot gently toward you until you feel the front of that lower thigh open up."
  - "Open your left arm out to the side and turn your gaze softly that way."
  - "Find your edge — about 60–80% of your capacity, a real stretch but never forcing — then be still and let the shape do the work. Let the twist find its own depth rather than cranking; drawing the foot toward you is where the pose turns more active, so ease the pressure if it starts to feel effortful."
  - "If the twist bites in the lower back, tuck a cushion under the top knee to take the strain."
transitions_in:
  - "From a Reclined Twist with knees dropped to one side, extend the bottom leg long and reach the top hand back for the bottom foot."
transitions_out:
  - "Come out slowly — release the foot, draw the knees back to centre, and rest on your back before changing sides."
counterposes:
  - "Lie on your back with knees bent and let the spine and hips re-centre."
  - "A gentle knees-to-chest hug to settle the lower back."
cautions:
  - "Ease off if you have disc trouble — keep the twist gentle and let a cushion under the knee take the strain rather than deepening the rotation."
  - "Go easy on the knee of the extended leg; hold the ankle or use a strap rather than cranking the foot toward the hip."
  - "Stop pulling on the foot if you feel anything sharp in the knee or lower back."
faq:
  - q: "What does Cat Pulling Its Tail stretch?"
    a: "It's a reclining twist with an extra opening for the front of the body. As the spine and ribcage wring gently, catching the foot of the bent top leg draws a stretch through the quadriceps and hip flexor of that thigh, while the lower back and outer hip soften into the twist."
  - q: "How long do you hold Cat Pulling Its Tail in Yin Yoga?"
    a: "Held as a quiet reclining twist, 2–5 minutes per side works well. If you actively draw the foot toward you the pose becomes more active and yang-like, so keep those versions shorter — around a minute — or release the pull and simply rest in the twist."
  - q: "Is Cat Pulling Its Tail safe if you have lower-back or disc issues?"
    a: "Go gently. Keep the twist mild and let a cushion under the top knee take the strain rather than deepening the rotation. Many people won't be able to draw the foot in at all, and that's fine — the twist alone is the pose. Stop if anything feels sharp in the back or the knee."
  - q: "What's the difference between Cat Pulling Its Tail and a Reclined Twist?"
    a: "A Reclined Twist simply lets both knees drop to one side with the legs relaxed. Cat Pulling Its Tail keeps that twist but extends the bottom leg and catches its foot with the opposite hand, adding a quad and hip-flexor stretch and a mild backbend — a twist and a front-line opener in one shape."
related_poses: ["reclined-twist", "twisted-dragon", "saddle", "dragon", "reclined-swan"]
youtube_video_id: ""
images:
  - "/poses/cat-pulling-its-tail.jpg"
seo_title: "Cat Pulling Its Tail in Yin Yoga: Benefits & How-To"
seo_description: "Cat Pulling Its Tail is a reclined twist variation that adds a quad and hip-flexor stretch. Cues, hold times and gentle disc-safe cautions."
---

<!-- Original copy in Katie's voice, grounded in Bernie Clark, The Complete Guide to
Yin Yoga, and Paul Grilley. Contraindications to be verified before publishing as advice. -->
