---
name_en: "Reclined Hamstring Stretch"
slug: "reclined-hamstring"
also_known_as: ["Reclined Hand-to-Big-Toe", "Supta Padangusthasana (yin variation)", "Reclined Leg Stretch"]
target_areas: ["hamstrings", "calves", "lower back"]
meridians: ["urinary bladder", "kidney"]
accessibility: "beginner"
hold_time: "2–4 minutes per side"
props: ["strap"]
hold_seconds: 150
benefits:
  - "A long, supported release for the hamstrings and calves, with the floor taking all the weight so the whole back of the leg can soften."
  - "Because you're lying down, the lower back and neck stay completely at ease — a gentler way to open the hamstrings than a seated forward fold."
  - "A strap does the reaching for you, so there's no rounding or straining to hold on."
  - "Calming and grounding — an easy shape to stay in and breathe."
cues:
  - "Come as you are. Lie on your back with both knees bent, feet on the floor."
  - "Hug one knee in, loop a strap (or a belt or towel) around the ball of that foot, and hold an end in each hand."
  - "Slowly extend the leg up toward the ceiling, only as far as the hamstring happily allows — a soft bend in the knee is completely fine and often kinder."
  - "Let your shoulders, head and hips stay heavy on the mat. Walk your hands down the strap only to the point of a real but comfortable stretch — about 80% of your edge."
  - "The lower leg can stay bent with the foot on the floor, or extend along the mat for a little more — whichever keeps your lower back at ease."
  - "Relax your hands, your jaw and your belly. Settle in and breathe, then switch sides."
transitions_in:
  - "From lying on your back — hug one knee in and loop the strap around the foot"
  - "From Happy Baby — extend one leg up with the strap"
transitions_out:
  - "Bend the knee, slip the strap off, and lower the foot to the floor."
  - "Hug both knees into the chest to rebalance before the second side."
counterposes:
  - "Wind-Relieving Pose (knees to chest)"
  - "A gentle Reclined Twist"
cautions:
  - "Keep a soft bend in the knee — never lock it straight, especially if you're very flexible or have hypermobile joints."
  - "If your lower back lifts or complains, bend the resting leg and place that foot on the floor to keep the pelvis grounded."
  - "Ease off if you feel any pulling behind the knee or in the sciatic line down the leg; this shape should feel long and steady, never sharp."
faq:
  - q: "Do I need a strap for the reclined hamstring stretch?"
    a: "A strap makes it much easier and more restful — it does the reaching so you can lie back and relax rather than straining to hold your foot. A belt, dressing-gown tie or towel works just as well."
  - q: "How long should I hold it?"
    a: "Around 2 to 4 minutes per side in Yin. Settle in, keep a soft bend in the knee, and let the back of the leg slowly release rather than forcing it."
  - q: "What if my hamstrings are really tight?"
    a: "Bend the raised knee generously and hold the strap higher up — you'll still get a deep, useful release. Flexibility comes with time and patient, regular practice, never from pushing."
related_poses: ["banana", "caterpillar", "legs-up-the-wall", "happy-baby"]
youtube_video_id: ""
images:
  - "/brand/pose-placeholder.png"
seo_title: "Reclined Hamstring Stretch in Yin Yoga: Benefits & How-To"
seo_description: "A supported, lying-down yin hamstring stretch using a strap — releases the hamstrings and calves while the lower back rests. Cues, benefits, hold time, cautions."
---

<!-- Original copy in Katie's voice, grounded in Bernie Clark, The Complete Guide to
Yin Yoga, and Paul Grilley. Contraindications to be verified before publishing as advice. -->
