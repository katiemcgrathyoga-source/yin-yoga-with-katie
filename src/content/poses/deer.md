---
name_en: "Deer"
slug: "deer"
also_known_as: []
target_areas: ["hips", "outer hips", "inner thighs", "lower back"]
meridians: ["liver", "gall bladder", "kidney", "spleen"]
accessibility: "beginner"
hold_time: "2–4 minutes per side"
props: ["cushion", "blanket"]
benefits:
  - "Opens both sides of the hip at once — one thigh in external rotation, the other in internal rotation — in a shape that is remarkably kind to the knees."
  - "A gentle way to explore internal hip rotation, which many Yin poses don't reach, so it balances the deeper outer-hip work."
  - "Stimulates the liver and gall bladder lines through the inner and outer thighs, and the kidney and spleen lines through the hips."
  - "Makes a soothing counter after a strong outer-hip pose like Shoelace or Swan, or a resting place between sides."
cues:
  - "Come to sitting with your knees bent. Take one shin out in front of you and swing the other shin around behind you, so both knees rest at roughly a right angle — front leg externally rotated, back leg internally rotated."
  - "Settle both sit bones down. If the back-leg hip lifts, slide a cushion under the front-leg sit bone so you feel level."
  - "You can simply stay upright here, tall through the spine, feeling both hips. That alone is a complete pose."
  - "To add more, turn toward the front shin and fold gently forward over it, or twist away from it — let your body choose which direction has something to release."
  - "Find your edge — about 80% of your capacity, a real stretch but never forcing. Be still and let the shape do the work."
  - "Come out slowly and switch the legs for the second side."
transitions_in:
  - "From a cross-legged seat, sweep one shin forward and the other behind you."
  - "From Shoelace, unstack the knees and bring the shins into the front-and-back Deer position."
transitions_out:
  - "Come out slowly. Bring the back shin around to the front and extend both legs forward to release."
  - "Pause, then reverse the shins for the other side."
counterposes:
  - "Extend both legs and lean back on your hands to let the hips settle."
  - "A gentle seated twist in each direction."
  - "Rest in Corpse pose once both sides are complete."
cautions:
  - "This shape is usually gentle on the knees, but if either knee feels any twist or pinch, widen the angle of the legs or slip a cushion under the tender knee."
  - "Sit up on a folded blanket if the lower back rounds uncomfortably or one hip won't reach the floor."
  - "If you fold forward, let the spine round softly — but with disc trouble or sciatica, stay upright and simply enjoy the rotation."
  - "Keep sensation in the hips, not the knee joints. Ease back if a knee objects."
related_poses: ["shoelace", "folded-half-shoelace", "swan", "sleeping-swan", "easy-pose"]
youtube_video_id: ""
images:
  - "/poses/deer.jpg"
seo_title: "Deer Pose in Yin Yoga: Benefits & How-To"
seo_description: "Deer is a knee-friendly Yin Yoga pose with shins front and back that gently opens both hips through internal and external rotation, upright or folding forward."
---

<!-- Original copy in Katie's voice, grounded in Bernie Clark, The Complete Guide to
Yin Yoga, and Paul Grilley. Contraindications to be verified before publishing as advice. -->
