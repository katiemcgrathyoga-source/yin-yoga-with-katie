import courseMaps from './courseRoutineMaps.json';
...
/**
 * Course routines: generated by scripts/gen-course-maps.mjs from what each
 * routine's poses work, caption included. Same figure, same files layout.
 */
const COURSE_MAPS: Record<string, { works: string }> = courseMaps;

export const routineMap = (slug: string) => {
  const entry = ROUTINE_MAPS[slug] ?? COURSE_MAPS[slug];
  return entry
    ? { src: `/bodymap/routines/${slug}.webp`, photo: `/bodymap/routines/${slug}.jpg`, works: entry.works }
    : null;
};
